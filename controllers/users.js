...
exports.registerUser = async (request, response) => {
  ...
  generateReferralCode().then((result) => {
...
exports.login = (request, response) => {
  Users.findOne({ where: { email: request.query.email } }).then((results) => {
    if (results === null) {
      response.status(400).json({
    ...
    bcrypt.compare(request.query.password, results.password).then((result) => {
      if (result) {
        Users.update(
          { lastLogin: new Date() },
...
exports.getUser = (_request, response) => {
  response.status(200).json({
    message: "ON USERS ROUTES"
  });
};

exports.deleteUser = (_request, response) => {
  response.status(200).json({
    message: "ON USERS ROUTES DELETE"
  });
};

exports.putUser = (_request, response) => {
  response.status(200).json({
